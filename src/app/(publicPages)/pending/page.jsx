...
const SEND_LOGIN_REASON = 'برای ارسال فرم‌های ذخیره‌شده باید وارد حساب دانشجویی خود شوید.';
...
export default function PendingPage() {
  ...
  // Split from the guard on purpose: requireAuth resumes THIS function after a
  // successful login, so it must not re-check auth itself or it would bounce
  // the sheet straight back open.
  ...
  const handleSendOne = (record) => {
    if (!isOnline) {
      toast.error('برای ارسال به اینترنت نیاز است');
      return;
    }
    // The queue is local, so guests keep their drafts; sending them is what
    // needs an account.
    requireAuth(() => sendOne(record), SEND_LOGIN_REASON);
  };
  ...
  const handleSendAll = () => {
    ...
    requireAuth(sendAll, SEND_LOGIN_REASON);
  };

  return (
    <div className="flex flex-col min-h-[calc(100dvh-56px)]">
      {/* Header */}
      <div className="sticky top-14 z-30 bg-surface border-b border-stroke-soft">
        ...
        {loading ? (
          ...
          items.map((record) => {
            ...
            return (
              <div
                key={record.id}
                className="rounded-xl border border-stroke-soft bg-surface p-3"
              >
      ...
      {/* Bulk send footer. 4rem clears the nav bar; the extra 1.5rem clears the
          centre button riding above it, which would otherwise cover — and steal
          taps from — the bottom of this button. */}
      {items.length > 0 && (
        <div className="fixed inset-x-0 bottom-[calc(5.5rem+env(safe-area-inset-bottom,0px))] z-40 flex justify-center">
          <div className="w-full max-w-[480px] bg-surface/95 backdrop-blur-sm border-t border-stroke-soft p-4">
            <Button
              type="button"
