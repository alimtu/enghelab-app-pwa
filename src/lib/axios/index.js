...
import { LOGIN_OP, AUTH_TOKEN_KEY } from '../auth/constants';
...
class HttpService {
  ...
  #isAuthRequest(config) {
    ...
    return op === 'm_login' || op === 'm_verify' || op === LOGIN_OP;
  }

  // The app stays browsable when a session expires: drop the stale token and
  // let AuthProvider offer the login sheet instead of wiping storage and
  // bouncing to a login page.
  #handleAuthFailure() {
    if (typeof window === 'undefined') return;
    // Only a real session can expire. A signed-out visitor hitting a protected
    // op should just see the request fail, not a "your session ended" sheet.
