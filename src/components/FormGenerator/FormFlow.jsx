...
export default function FormFlow({ form, onBack }) {
  ...
  const doSubmit = async () => {
    ...
    requireAuth(sendAnswers, 'برای ثبت این فرم باید وارد حساب دانشجویی خود شوید.');
  };
  ...
  return (
    ...
    <div className="flex flex-col min-h-[calc(100dvh-56px)]">
      <div className="flex-1 pb-24">
            ...
            {steps.map((s, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center">
                  ...
                  {/* Label */}
                  <span className={`text-[10px] mt-1.5 text-center leading-tight max-w-[64px] truncate ${active ? 'text-primary-600 font-medium' : done ? 'text-grey-500' : 'text-grey-400'
                    }`}>
                    {s.title}
                  </span>
                </div>
        ...
        <div className="space-y-5 p-4">
          {allSections.length === 0 ? (
            <p className="text-sm text-grey-500 text-center py-6">بدون فیلد</p>
          ) : isRepeatable ? (
      ...
      <div className="fixed bottom-0 inset-x-0 z-50 flex justify-center safe-bottom">
        <div className="w-full max-w-[480px] bg-surface/95 backdrop-blur-sm border-t border-stroke-soft p-4 flex gap-3">
          {!isFirst && (
