...
// Values live in globals.css (:root / .dark), so these follow the active theme.
const COLORS = [
...
function ChartCard({ title, children }) {
  return (
    <div className="rounded-2xl border border-stroke-soft bg-surface overflow-hidden">
...
function StatCard({ label, value, color }) {
  return (
    <div className="rounded-2xl border border-stroke-soft bg-surface p-4 flex flex-col items-center gap-1">
      <div className="size-10 rounded-full flex items-center justify-center" style={{ backgroundColor: `color-mix(in srgb, ${color} 12%, transparent)` }}>
...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-surface border border-stroke-soft rounded-lg shadow-sm px-3 py-2 text-xs" dir="rtl">
      {payload.map((p, i) => (
...
export default function ChartsPage() {
  return (
    <AuthGate
      ...
      title="آمار فرم‌ها"
      description="گزارش‌ها بر پایه اطلاعات حساب شما ساخته می‌شوند؛ برای دیدن آن‌ها وارد شوید."
    >
      <ChartsContent />
    </AuthGate>
  );
...
function ChartsContent() {
  ...
  // Either dataset missing means the page can only render half its charts, so
  // surface a retry rather than silently showing empty ones.
      ...
      {reqCharts.totalRequests > 0 && (
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            <StatCard label="کل درخواست‌ها" value={reqCharts.totalRequests} color="var(--chart-6)" />
            <StatCard label="فرم‌های دارای درخواست" value={reqCharts.totalForms} color="var(--chart-7)" />
          </div>

          <ChartCard title="تعداد درخواست به ازای هر فرم">
            <HorizontalList data={reqCharts.perForm} color="var(--chart-6)" unit="درخواست" />
          </ChartCard>
      ...
      {formCharts && (
        <>
          ...
          <div className="grid grid-cols-3 gap-3">
            <StatCard label="فرم" value={totalForms} color="var(--chart-1)" />
            <StatCard label="مرحله" value={totalSteps} color="var(--chart-2)" />
            <StatCard label="فیلد" value={totalFields} color="var(--chart-5)" />
          </div>

          <ChartCard title="تعداد مراحل هر فرم">
            <HorizontalList data={formCharts.stepsPerForm} color="var(--chart-1)" unit="مرحله" />
          </ChartCard>

          <ChartCard title="تعداد فیلدهای هر فرم">
            <HorizontalList data={formCharts.fieldsPerForm} color="var(--chart-2)" unit="فیلد" />
          </ChartCard>
                ...
                return (
                  <div key={d.name}>
                    ...
                    <div className="flex h-2.5 rounded-full overflow-hidden bg-grey-50">
                      {reqPct > 0 && (
                        <div className="bg-[var(--chart-4)] transition-all duration-500" style={{ width: `${reqPct}%` }} />
                      )}
                      {optPct > 0 && (
                        <div className="bg-[var(--chart-5)] transition-all duration-500" style={{ width: `${optPct}%` }} />
                      )}
                    </div>
                  </div>
                );
