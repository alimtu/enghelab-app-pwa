        ...
        {form.steps.map((step) => {
          ...
          return (
            <AccordionItem key={step.stepId} value={`step-${step.stepId}`}>
              <AccordionTrigger className="bg-grey-100 px-3 rounded-md text-sm font-semibold">
                {step.title}
              </AccordionTrigger>
              <AccordionContent className="px-1 pt-3">
                {step.discription && (
                  <p className="text-xs text-grey-500 mb-3">{step.discription}</p>
                )}
                <div className="space-y-4">
                  ...
                  {allSections.length === 0 && (
                    <p className="text-xs text-grey-400">بدون فیلد</p>
                  )}
                </div>
              </AccordionContent>
            </AccordionItem>
