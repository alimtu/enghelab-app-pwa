    ...
    <div className="Footer-img relative z-0 w-full mx-auto text-white">
      <div className="absolute inset-0 w-full h-full">
        <Image
          ...
          alt="ساختمان دانشگاه جامع انقلاب اسلامی"
          fill
          priority
      ...
      <div className="relative pb-16 z-20 grid grid-col-1 lg:grid-cols-3 px-3">
        <div className="useful-links mr-4 ml-4 mt-2">
          ...
          <ul className="flex flex-col gap-7 list-disc mr-4">
            {usefulLinks.map((item, index) => (
              <li key={index}>
                <Link href="#" className="hover:text-accent-200 text-sm">
                  {item}
                </Link>
              </li>
            ))}
              ...
              return (
                <div key={item.id} className="space-y-2">
                  <div className="flex gap-2">
                    ...
                    <span className="hover:text-accent-200 text-sm">{item.title}</span>
                  </div>
                  <a
                    href={getContactHref(item)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-white hover:text-accent-200 text-sm"
                  >
                    {item.content}
                  </a>
                </div>
          ...
          <div className="map flex justify-center pt-9 rounded-2xl pb-4 ">
            <div className="relative w-full h-[200px] z">
              <iframe
                ...
                marginHeight="0"
                marginWidth="0"
                title="موقعیت دانشگاه جامع انقلاب اسلامی"
        ...
        <div className="text-center absolute px-12 bottom-0 w-full z-30">
          <p className="p-3 m-auto border-solid border-t-1 border-grey-500">
